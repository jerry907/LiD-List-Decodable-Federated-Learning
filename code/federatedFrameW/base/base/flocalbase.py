...
class lbase(metaclass=ABCMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.local_model = copy.deepcopy(list(self.model.parameters())) # a copy of lmodel params
    ...
    def save_mdl(self, names):
        for name in names:
            self.clone_model_parameters(self.personalized_model, self.mdl_param[name])

    def load_data(self, train_data, test_data, batch_size):
        self.train_samples = len(train_data)
        self.test_samples = len(test_data)
        self.trainloader = DataLoader(train_data, batch_size, shuffle=True) # shuffle：set to True to have the data reshuffled at every epoch (default: False)
        self.testloader = DataLoader(test_data, batch_size, shuffle=True)
        self.trainloader_full = DataLoader(train_data, len(train_data))  # all train_data as one batch
    ...
    def set_model_parameters(self, model):
        if isinstance(model, nn.Parameter):
            ...
            for idx, model_param in enumerate(self.model.parameters()):
                model_param.data = model[idx].clone()
        else:
            raise Exception('Setting model parameters, unproper type error')
    ...
    def set_local_parameters(self, model):
        if isinstance(model, nn.Parameter):
            ...
            for idx, model_param in enumerate(self.local_model):
                model_param.data = model[idx].clone()
        else:
            raise Exception('Setting local model parameters, unproper type error')
    ...
    def test(self):
        '''return accumulated test_acc, test_loss, tested by the whole local test dataset'''
        self.model.eval()
        test_acc = 0
        test_loss = 0
    ...
    def train_error_and_loss(self):
        '''return accumulated train_acc, train_loss, trained by the whole local training dataset'''
        self.model.eval()
        train_acc = 0
        loss = 0
    ...
    def get_next_temp_train_batch(self):
        try:
            (X, y) = next(self.temp_iter_train)
            # print('get_next_temp_train_batch !')
        except StopIteration:
            self.temp_iter_train = iter(self.temp_trainloader)
            (X, y) = next(self.temp_iter_train)
            # print('temp_iter_train stopIteration and reset')
        return (X.to(self.device), y.to(self.device))
