...
class gbase(metaclass=ABCMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mdl_param = {'global_acc': copy.deepcopy(list(self.model.parameters()))
            ...
            , 'train_loss': copy.deepcopy(list(self.model.parameters()))
                          } # the four keys are misnamed

    def save_mdl2p(self, nid, file_name):
        '''save model params to path, optionally'''
        for name in self.mdl_param.keys():
            if not os.path.isdir('mdl_param/' + file_name):
                ...
                os.mkdir('mdl_param/' + file_name + '/' + name)
            path = 'mdl_param/' + file_name + '/' + name + '/' + '[' + str(nid) + '-' + str(self.id) + ']' + '.txt'
    ...
    def send_parameters(self):
        '''
        send gmodel to candidate_locals
        '''
    ...
    def aggregate_parameters(self, beta=1):
        ...
        # store previous parameters then reset gmodel
        if beta != 1:
        ...
        # aggregate sampled_locals (weighted average) to gmodel
        for local in self.sampled_locals:
            self.add_parameters(local, local.train_samples / total_train)

        # aggregate avergage gmodel with previous gmodel using parameter beta
        if beta != 1:
    ...
    def select_locals_uniform(self, num_aggregate_locals):
        ...
        return np.random.choice(self.candidate_locals, num_aggregate_locals, replace=False)
    ...
    def test(self):
        ...
        for c in self.candidate_locals:
            ct, tl, ns = c.test()
            tot_correct.append(ct * 1.0) # total acc of all samples on local c
    ...
    def save_mdl_g(self, names):
        '''clone/save gmodel params to self.mdl_param, single params to multi-name'''
        for name in names:
            self.clone_model_parameters(self.model.parameters(), self.mdl_param[name])
    ...
    def save_mdl_p(self, names):
        '''save lmodel params for all candidate_locals'''
        for c in self.candidate_locals:
            c.save_mdl(names)
    ...
    def evaluate(self):
        ...
        glob_acc = np.sum(stats[2]) * 1.0 / np.sum(stats[1])  # test acc pre sample 
        train_acc = np.sum(stats_train[2]) * 1.0 / np.sum(stats_train[1])  # train acc pre sample
        test_loss = sum([x * y for (x, y) in zip(stats[1], stats[3])]).item() / np.sum(stats[1])  # weighted avg of test loss among all locals
        train_loss = sum([x * y for (x, y) in zip(stats_train[1], stats_train[3])]).item() / np.sum(stats_train[1])

        l2sv = []  # record model params if making progress
        ...
        self.save_mdl_g(l2sv)  # save gmodel params
