...
class CentralizedFL(tbase):
    ...
    def gen_globals(self):
        model = copy.deepcopy(self.model)
        hyperparams = copy.deepcopy(self.hyperparams)
        fg = self.gImp(id=0, model=model, hyperparams=hyperparams)
        self.fglobals.append(fg)
        # print(vars(fg))
        ...
        for fn in self.fnodes:
            for fg in fn.fglobals:
                if ((len(fg.rs_glob_acc) != 0)
                    ...
                    fg.save_mdl2p(fn.id, self.res_file_name()) # save model to path

                    self.save_aim(re.split('_\d+$', file_name)[0], train_acc=fg.rs_glob_acc, train_loss=fg.rs_train_loss,
    ...
    def eval_global_model(self):
        ler = []
        for fn in self.fnodes:
            ler.append(fn.eval_global()) # ler.append([glob_acc, train_acc, train_loss])
